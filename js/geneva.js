...
Geneva.Session.prototype = {
    ...
    populate: function(numChromosomes, numNotes) {
        for (i=0; i<numChromosomes; i++) {
            var notes = this.generateNotes(numNotes);
            this.chromosomes[i] = new Geneva.Chromosome(notes);
        }
    },

    generateNotes: function(n, mode) {
        var notes = [];
...
session.populate(Geneva.DEFAULT_CHROMOSOMES, Geneva.DEFAULT_NOTES);
